...
- the published API image keeps its heavy Rivet/Yarn builder stage on Docker BuildKit's `BUILDPLATFORM` while the final runtime stage stays on `TARGETPLATFORM`. This avoids running the upstream Yarn link/build tooling under QEMU during `linux/arm64` GitHub image publishes, where native package install hooks can fail with illegal-instruction crashes. Before copying build output into the runtime stage, the image focuses the Rivet install to production dependencies for `@valerypopoff/rivet2-core` and `@valerypopoff/rivet2-node`, prunes wrapper API dev dependencies, and recreates the generated Rivet package overlays so final images do not carry the build-only native toolchain.
