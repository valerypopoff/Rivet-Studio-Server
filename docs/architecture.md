...
- The iframe renders the upstream Rivet app plus wrapper-provided overrides and `EditorMessageBridge`, which coordinates open/delete/replay commands plus parent-page save requests with the dashboard via `window.postMessage`. Save completion, active project changes, and open-project counts are forwarded through `RivetAppHost` callbacks instead of wrapper-owned copies of upstream save/menu hooks.
...
- Hosted editor mounting uses Rivet 2.0's `RivetAppHost` seam from `rivet/packages/app/src/host.tsx`. The wrapper passes `/ws/executor/internal` as `executor.internalExecutorUrl`, so Node executor mode reuses Rivet's upstream executor-session and remote-executor transport. The wrapper also forwards `RivetAppHost` lifecycle callbacks to the dashboard bridge for save/open/active-project state. Upstream Rivet owns internal executor classification, debugger handoff, save transitions, and menu command dispatch; the wrapper must not alias `useExecutorSession`, `useRemoteDebugger`, `useGraphExecutor`, `useRemoteExecutor`, `useSaveProject`, or `useMenuCommands`.
- Hosted opened-project state follows Rivet 2.0's split model: tab metadata stays in `projectsState.openedProjects`, while full project content stays in `openedProjectSnapshotsState`. Wrapper hosted hooks must keep that shape when opening or syncing workflow projects; otherwise upstream tab close/switch behavior can re-add closed projects or lose in-memory project snapshots. Direct workflow opens pass the freshly loaded snapshot into the loader so the new tab can become active immediately without waiting for atom persistence to round-trip.
- Hosted Tauri API shims must cover every upstream `@tauri-apps/api/*` subpath used by Rivet's browser bundle; currently that includes the `tauri` subpath for guarded native `invoke` calls. Wrapper module overrides for Rivet internals are importer-scoped to `rivet/packages/app/src` so unrelated wrapper or dependency files with matching basenames are not rewritten accidentally. Prefer upstream provider seams such as environment and path policy providers over wrapper copies of upstream modules.
