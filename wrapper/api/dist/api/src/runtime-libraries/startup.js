...
import { emptyManifest, ensureDirectories, getRootPath, nextReleaseId, readActiveRelease, readManifest, releasesDir, writeActiveRelease, writeManifest, } from './manifest.js';
const MAX_RELEASES_TO_KEEP = 5;
export async function reconcileRuntimeLibraries() {
...
function migrateCurrentReleaseIfNeeded() {
    const root = getRootPath();
...
function buildManifestFromRelease(releasePath, previousManifest) {
...
function cleanupOldReleases(activeReleaseId) {
    try {
