...
import { isEnvAllowed, validatePath, getAppDataRoot } from '../security.js';

export const compatRouter = Router();

// POST /api/compat/invoke — dispatch by command name
compatRouter.post('/invoke', async (req, res) => {
  try {
    ...
    switch (command) {
      ...
      case 'read_relative_project_file': {
        const relativeFrom = args.relativeFrom as string;
        const projectFilePath = args.projectFilePath as string;
      ...
      case 'extract_package_plugin_tarball': {
        const tarPath = args.path as string;
        if (!tarPath) {
